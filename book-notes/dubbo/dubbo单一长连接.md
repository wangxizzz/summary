...
为了要支持高并发场景，一般是服务提供者就几台机器，但是服务消费者有上百台，可能每天调用量达到上亿次！此时用长连接是最合适的，就是```跟每个服务消费者维持一个长连接就可以，可能总共就 100来 个连接```。然后后面直接基于长连接 NIO 异步通信，可以支撑高并发请求。**每个consumer和provider都会建立一个长连接。**

<img src="../../imgs/nio单一长连接.jpg" height=300px>
